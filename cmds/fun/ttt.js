...
module.exports.run = async (client, message, args) => {
  ...
  message.delete()
    if (!enemy) return message.channel.send("Пинг бота или игрока/себя").then(msg => msg.delete(5 * 1000));
            ...
            const callback = async (id, pl, r) => {
                if(enemy.bot) {
                    ...
                    if(enemy.id !== message.author.id) {
                        if(round % 2 !== 0 && id === enemy.id) return r.remove(id);
                        if(round % 2 === 0 && id === message.author.id) return r.remove(id);
                    }
                }
                ...
                if(win) {
                  ...
                  if (message.author.bot)return
                    return message.channel.send(`<@${win}> Выиграл(-a)!`)
                } else if(!win && round === 10) {
                    if(player2) player2.stop();
                    player1.stop();
                    msg.clearReactions();
                  if (message.author.bot)return
                    return message.channel.send(`Ничья!`)
                }
                await r.removeAll();
                const board = [];
                for(let key in  tb) {
                ...
                if(enemy.bot) {
                    if(round % 2 === 0) aiCalculate(board.map((row, index) => reactions.includes(row) ? index : row), tb, player1, reactions, emojis, msg);
                }
            };
