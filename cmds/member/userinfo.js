...
module.exports.run = async(bot, message, args) => {
    ...
    let embed = new RichEmbed()
        .setTitle(argsUser.username,false)
        .setDescription(game, false)
       .addField('Устройство',`${message.author.presence.clientStatus.desktop ? "Компьютер": ""} ${message.author.presence.clientStatus.mobile ? "Телефон": ""} ${message.author.presence.clientStatus.web ? "Браузер": ""}`,true)
      //  .addField('Значок',`${message.author.flags.toArray().join(" ")}`,false) 
     //   .addField('Устройство',`${message.author.presence.clientStatus}`,false)
    //  .addField('Значок',`${message.author.flags.toArray()}`,false)
        .addField('Дата регистарции', `${strftime('%d.%m.%Y в %H:%M', new Date(argsUser.createdTimestamp))}\n(${diff1} дн. назад)`, false)
        .addField('Дата вступления', `${strftime('%d.%m.%Y в %H:%M', new Date(message.guild.member(argsUser).joinedTimestamp))}\n(${diff2} дн. назад)`, false)
        .addField('Роли', message.guild.member(argsUser).roles.filter(r => r.id != message.guild.id).map(role => role.name).join(' ** **, ') || 'Не имеет', false)
        .setColor(message.guild.member(argsUser).displayHexColor)
        .setTimestamp()
        .setThumbnail(argsUser.avatarURL)
        .setFooter(`ID: ${argsUser.id}`)
    message.delete().catch();
    await message.channel.send(embed)//.then(msg => msg.delete(30 * 1000));
}
